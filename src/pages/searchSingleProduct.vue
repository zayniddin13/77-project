    ...
    <div v-show="!loading" class="body md:flex gap-10 block">
      <div v-if="product" class="md:w-8/12 w-full">
        ...
        <div v-if="product" class="px-6 py-5 bg-white rounded-2xl my-4">
          ...
          <div
            class="font-ibmPlex text-black-1 font-normal text-base !break-words"
          >
            {{ product.description }}
          </div>
        </div>
      ...
      <div v-if="product" class="img_adress max-w-xs w-full pb-4">
        ...
        <div v-if="product.address" class="bg-white my-4 rounded-2xl">
          <div class="p-4">
            ...
            <div class="flex gap-2.5">
              <span
                class="icon-location text-main-blue text-3xl font-semibold"
              ></span>
        ...
        >
          <template #suffix>
            <span class="icon-download text-xl text-main-blue font-bold"></span>
          </template>
        </EnterButton>
      </div>
    </div>
...
async function dataFetch() {
  ...
  try {
    ...
    // if (product.value.seller.phone_number) {
    //   number.value = product.value.seller.phone_number.replace(
    //     /(\d{3})(\d{2})(\d{3})(\d{2})(\d{2})/,
    //     "$1 $2 $3 $4 $5"
    //   );
    // } else {
    //   number.value = "+998 343 33 23";
    // }
